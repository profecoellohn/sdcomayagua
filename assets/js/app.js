...
const ahora = new Date();
        ...
        function cargarRestoDatosLocales() {
            ...
            if(historialVentas.length > 0) {
                ventasTotales = historialVentas.reduce((s, v) => s + Number(v.total || 0), 0);
            }
        }
        ...
        function actualizarResumenVentas() {
            const total = historialVentas.reduce((s, v) => s + Number(v.total || 0), 0);
            const countEl = document.getElementById('ventas-count');
            const totalEl = document.getElementById('ventas-total');
            if(countEl) countEl.innerText = historialVentas.length;
        ...
        function renderVentas() {
            ...
            const ventasFiltradas = historialVentas.filter(v => {
                const texto = `${v.cliente || ''} ${v.telefono || ''} ${v.destino || ''} ${v.paqueteria || ''}`.toLowerCase();
            ...
            ventasFiltradas.forEach((venta) => {
                ...
                cont.innerHTML += `
                    <div class="item-lista venta-card">
                        <div class="venta-top">
                            <div>
                                <b>${venta.cliente || 'Consumidor Final'}</b>
                                <p class="venta-meta">${fechaTxt}<br>📍 ${venta.destino || 'Destino no especificado'} · 📦 ${venta.paqueteria || 'N/A'}</p>
                            </div>

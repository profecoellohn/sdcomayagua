...
const ahora = new Date();
        ...
        function refrescarCatalogoVisual() {
            const cat = ['Todos', ...new Set(baseDeDatos.map(p => p.categoria).filter(Boolean))];
            if(!cat.includes(categoriaActivaCatalogo)) categoriaActivaCatalogo = 'Todos';
            document.getElementById('contenedor-filtros').innerHTML = cat.map(c => `<button class="btn-cat ${c===categoriaActivaCatalogo?'activo':''}" onclick="filtrar('${c}', this)">${c}</button>`).join('');
        ...
        function renderizarProductos(arr) {
            ...
            arr.forEach(prod => {
                ...
                cont.innerHTML += `
                    <article class="card-prod shop-card ${estadoClase}" data-stock-state="${estadoClase}" onclick="abrirModal('${prod.id}')">

...
// --- V95: SIN ADMIN / SIN PIN ---
(function(){
  function sdcEntrarDirectoV95(){
    try {
      sessionStorage.setItem('sdc_admin_ok','1');
      localStorage.setItem('sdc_admin_ok','1');
  ...
  window.sdcEntrarDirectoV95 = sdcEntrarDirectoV95;
  sdcEntrarDirectoV95();
  document.addEventListener('DOMContentLoaded', sdcEntrarDirectoV95);
  window.addEventListener('load', sdcEntrarDirectoV95);
  setTimeout(sdcEntrarDirectoV95, 50);
  setTimeout(sdcEntrarDirectoV95, 700);
})();
...
function actualizarPreviewCategoriaNuevo() {
    ...
    if(asset) {
        ...
        function cerrarSesionAdmin() {
            ...
            if(typeof mostrarToast === 'function') mostrarToast('Sistema activo.');
        }
...
<div class="hr"></div><div class="row total"><span>TOTAL</span><b>Lps. ${Math.round(Number(venta.total || 0))}</b></div><div class="hr"></div><div class="center">Gracias por su compra</div></div></body></html>`;
        ...
        // v94: no cargar catálogo/ventas mientras el login está activo.
        // Esto evita que el navegador se quede en "Esta página no responde" antes de escribir el PIN.
        let sdcAppInitStartedV94 = false;
        function sdcIniciarAppDespuesLoginV94() {
            if(sdcAppInitStartedV94) return;
            if(document.body.classList.contains('login-locked') && !adminEstaAutenticado()) return;
            sdcAppInitStartedV94 = true;
        ...
        setTimeout(sdcIniciarAppDespuesLoginV94, 300);
        setTimeout(sdcIniciarAppDespuesLoginV94, 1200);
        document.getElementById('login-form')?.addEventListener('submit', () => {
            setTimeout(sdcIniciarAppDespuesLoginV94, 250);
        }, true);
        const sdcLoginObserverV94 = new MutationObserver(() => {
            if(!document.body.classList.contains('login-locked')) {
                setTimeout(sdcIniciarAppDespuesLoginV94, 250);
            }
        });
        try { sdcLoginObserverV94.observe(document.body, { attributes: true, attributeFilter: ['class'] }); } catch(e) {}
...
(function(){
    function syncMobileCatalogSearchButtons() {
        ...
        if(window.innerWidth <= 860) {
            ...
            const originRowMobile = document.querySelector('.catalog-search-toggle-row');
            if(originRowMobile) originRowMobile.setAttribute('hidden', 'hidden');
    ...
    function syncMobileHeaderAndCatalogV63() {
        ...
        if(btn && originRow && mainRow && categorySelect) {
            ...
            if(window.innerWidth <= 860) {
                ...
                if(originRow) originRow.removeAttribute('hidden');
                if(btn.parentElement !== originRow) {
                    originRow.appendChild(btn);
                }
                btn.classList.remove('mobile-search-trigger');
...
// v92 fallback login function
window.sdcUnlockAdminV92 = window.sdcUnlockAdminV92 || function(){
  try { sessionStorage.setItem('sdc_admin_ok','1'); localStorage.setItem('sdc_admin_ok','1'); } catch(e) {}
  try {
    window.SDC_ADMIN_OK = true;
    document.documentElement.classList.add('sdc-admin-unlocked','sdc-splash-skip');
    ...
    if(login){ login.classList.add('oculto'); login.setAttribute('hidden','hidden'); login.style.setProperty('display','none','important'); }
  } catch(e) {}
};

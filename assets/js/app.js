...
const ahora = new Date();
        ...
        function renderizarProductos(arr) {
            ...
            arr.forEach(prod => {
                ...
                const badge = stk > 0 ? `<span class="tag-stock">${stk} en stock</span>` : `<span class="tag-stock" style="background: rgba(239, 68, 68, 0.92); color: #fff;">Agotado</span>`;
                const categoria = prod.categoria || 'General';
                const chipPromo = (prod.promos && prod.promos.length) ? `<span class="card-chip">Con promo</span>` : `<span class="card-chip" style="background: rgba(148, 163, 184, 0.14); border-color: rgba(148, 163, 184, 0.18); color: #e2e8f0;">Disponible</span>`;
                cont.innerHTML += `
                    <article class="card-prod" onclick="abrirModal('${prod.id}')">
        ...
        async function compartirFichaProducto() {
            ...
            sincronizarFichaProductoCompartir();
            try {
                const canvas = await html2canvas(document.getElementById('contenedor-ficha-producto'), { scale: 2, useCORS: true, backgroundColor: null });
        ...
        function generarImagenFactura() {
            ...
            html2canvas(document.getElementById('contenedor-factura'), { scale: 2, useCORS: true, backgroundColor: '#ffffff' }).then(c => {
                const link = document.createElement('a');
